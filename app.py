...
def login():
    if request.method == "POST":
        ...
        if user_data:
            user = user_data[0]
            # Password check is bypassed, just for show
...
def signup():
    if request.method == "POST":
        email = request.form.get("email")
        password = request.form.get("password")
        # Store email and hashed password (password is just for show)
...
def internship_form():
    if request.method == "POST":
        ...
        # Store skills and sectors in session for saving/applying
        session["user_skills"] = skills
        session["user_sectors"] = sectors
        # Update profile only if not dummy user
        if session["user_id"] != "123e4567-e89b-12d3-a456-426614174000":
            supabase.table('profiles').upsert({
                'user_id': session["user_id"],
                'education': education,
                'skills': skills,
        ...
        saved_ids = []  # For dummy, no saved
        return render_template("output.html", results=recommendations[:10], locations=all_locations, saved_ids=saved_ids)
    profile = {}  # For dummy, no profile
    return render_template("input.html", skills=all_skills, sectors=all_sectors, profile=profile)
